...
extern "C" {
    ...
    // unpack_thumb() would.
    int unpack_thumbnail_layer_source(LibRaw& RawProcessor) {
        const libraw_thumbnail_list_t& list = RawProcessor.imgdata.thumbs_list;
        const libraw_thumbnail_t& largest = RawProcessor.imgdata.thumbnail;
        ...
        if (largest_index < 0 ||
            ...
            return RawProcessor.unpack_thumb();
        }
        ...
        if (best_index >= 0 &&
                RawProcessor.unpack_thumb_ex(best_index) == LIBRAW_SUCCESS) {
        ...
        return best_index >= 0 ? RawProcessor.unpack_thumb_ex(largest_index)
    ...
    ThumbnailResult process_thumbnail(LibRaw& RawProcessor, void* cancel_token) {
        ...
        if (unpack_thumbnail_layer_source(RawProcessor) == LIBRAW_SUCCESS) {
            int errc = 0;
    ...
    EXPORT void get_embedded_jpeg(const char* file_path,
                                  ThumbnailResult* out) {
        if (!out) return;
    ...
    EXPORT void get_embedded_jpeg_from_buffer(uint8_t* buffer, int size,
                                              ThumbnailResult* out) {
        if (!out) return;
        ...
        *out = empty_thumbnail();
        ...
        LibRaw RawProcessor;
        if (RawProcessor.open_buffer(buffer, static_cast<size_t>(size)) !=
            LIBRAW_SUCCESS) {
            return;
        }
